    ...
    def get(self):
        ...
        if not query_month:
            ...
            query_statement = "SELECT DATE_FORMAT(abwtb.custom_time,'%%Y-%%m') AS `date`, abwtb.author_id, usertb.name, COUNT(*) AS `count` " \
                              ...
                              "ON (abwtb.author_id=usertb.id) AND (DATE_FORMAT(abwtb.custom_time,'%%Y-%%m-%%d') BETWEEN %s AND %s) GROUP BY abwtb.author_id, DATE_FORMAT(abwtb.custom_time,'%%Y-%%m') ORDER BY DATE_FORMAT(abwtb.custom_time,'%%Y-%%m') ASC;"
