var vm = new Vue({
	...
	methods:{
		goToTargetPage(flag){
			...
			var localThis = this;
			var hostServer = host + 'onduty-message/?page='+ requirePage+'&pagesize=30&utoken=' + token; 
		...
		getCurrentPageRecord(cPage){
			var localThis = this;
			var hostServer = host + 'onduty-message/?page='+ cPage +'&pagesize=30&utoken=' + token; 
			axios.get(hostServer)
			.then(function(resp){
				// console.log(resp)
				localThis.currentRecords = resp.data.records;
		...
		exportRecord(){
			this.exportDataUrl = host + 'onduty-message/export/?utoken=' + token + '&r=' + Math.random();
		},
		closeModify(){
		...
		modifyCurrentRecord(){
			...
			var server_url = host + 'onduty-message/' + this.operateRecordId + '/'
			var localThis = this;
		...
		deleteCurrentRecord(){
			...
			if(confirm("删除是不可恢复的,您确定要删除吗?")){
				var localThis = this;
				var server_url = host + 'onduty-message/' + this.operateRecordId + '/?utoken=' + token;
